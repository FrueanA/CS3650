...
int nufs_readdir(const char *path, void *buf, fuse_fill_dir_t filler,
  ...
  for (slist_t *curr = list; curr != NULL; curr = curr->next) {
    char entry_path[256];
    snprintf(entry_path, sizeof(entry_path), "%s/%s", path, curr->data);
    
    // Remove double slash for root
    if (strcmp(path, "/") == 0) {
      snprintf(entry_path, sizeof(entry_path), "/%s", curr->data);
    }
    
    rv = storage_stat(entry_path, &st);
    if (rv == 0) {
...
int nufs_fsync(const char *path, int isdatasync, struct fuse_file_info *fi) {
  blocks_sync();
  printf("fsync(%s) -> 0\n", path);
  return 0;
}
